<div class="main-sidebar sidebar-style-2">
    <aside id="sidebar-wrapper">
        <div class="sidebar-brand">
            <a href="index.html">My Blog</a>
        </div>
        <div class="sidebar-brand sidebar-brand-sm">
            <a href="index.html">Blog</a>
        </div>
        <ul class="sidebar-menu">
            <li><a class="nav-link" href="{{ route('home') }}"><i class="fas fa-fire"></i><span>Dashboard</span></a></li>
            <li class="dropdown {{ request()->is('post*') ? 'active' : '' }}">
                <a href="#" class="nav-link has-dropdown" data-toggle="dropdown"><i class="fas fa-book-open"></i>
                    <span>Post</span></a>
                <ul class="dropdown-menu">
                    <li><a class="nav-link" href="{{ route('post.index') }}">List Of Post</a></li>
                </ul>
               @if (Auth::user()->type == '1')
                <ul class="dropdown-menu">
                    <li><a class="nav-link" href="{{ route('post.trash') }}">List Of Trash Post</a></li>
                </ul>
               @endif
                
               
            </li>
            @if (Auth::user()->type == '1')
            <li class="dropdown {{ request()->is('category*') ? 'active' : '' }}">
                <a href="#" class="nav-link has-dropdown" data-toggle="dropdown"><i class="far fa-clipboard"></i>
                    <span>Category</span></a>
                <ul class="dropdown-menu">
                    <li><a class="nav-link" href="{{ route('category.index') }}">List Of Category</a></li>
                </ul>
            </li>
            <li class="dropdown {{ request()->is('tag*') ? 'active' : '' }}">
                <a href="#" class="nav-link has-dropdown" data-toggle="dropdown"><i class="far fa-bookmark"></i>
                    <span>Tag</span></a>
                <ul class="dropdown-menu">
                    <li><a class="nav-link"  href="{{ route('tag.index') }}">List Of Tag</a></li>
                </ul>
            </li>
            <li class="dropdown {{ request()->is('user*') ? 'active' : '' }}">
                <a href="#" class="nav-link has-dropdown" data-toggle="dropdown"><i class="far fa-user"></i>
                    <span>Users</span></a>
                <ul class="dropdown-menu">
                    <li><a class="nav-link"  href="{{ route('user.index') }}">List Of User</a></li>
                </ul>
            </li>
           @endif
           
        </ul>
    </aside>
</div>
